...
/**
 * Extra weight applied to a candidate ship placement for every "active hit"
 * (unsunk hit cell) that placement covers. A high multiplier makes the
 * density map gravitate toward completing a wounded ship even when the
 * orientation-aware target-mode walk has failed (e.g. two separate ship
 * fragments on the same row) — effectively a secondary target mode.
 */
const ACTIVE_HIT_WEIGHT = 10;
    ...
    for (let r = 0; r < BOARD_SIZE; r++) {
      for (let c = 0; c <= BOARD_SIZE - size; c++) {
        ...
        for (let i = 0; i < size; i++) {
          ...
          if (state === 'hit') activeHits++;
        }
        if (valid) {
          const weight = 1 + activeHits * ACTIVE_HIT_WEIGHT;
          for (let i = 0; i < size; i++) {
